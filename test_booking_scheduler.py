...
def booking_scheduler_with_sms_mock(mocker: MockerFixture):
    ...
    testable_sms_sender : SmsSender = mocker.Mock()
...
def booking_scheduler_with_mail_mock(mocker: MockerFixture):
    booking_scheduler = BookingScheduler(CAPACITY_PER_HOUR)
    testable_mail_sender : MailSender = mocker.Mock()
    booking_scheduler.set_mail_sender(testable_mail_sender)
    return booking_scheduler, testable_mail_sender

@pytest.fixture
def customer(mocker: MockerFixture):
    customer : Customer = mocker.Mock()
    customer.get_email.return_value = None
    return customer

@pytest.fixture
def customer_with_mail(mocker: MockerFixture):
    customer : Customer = mocker.Mock()
    customer.get_email.return_value = "jane.do@aaa.com"
    return customer
