...
CUSTOMER_WITH_MAIL = Customer("Fake name", "010-1234-5678", "jane.do@aa.com")
MONDAY_DATETIME = datetime.strptime("2024/06/03 17:00", "%Y/%m/%d %H:%M")
SUNDAY_DATETIME = datetime.strptime("2026/07/05 17:00", "%Y/%m/%d %H:%M")
...
def test_예약완료시_SMS는_무조건_발송(booking_scheduler_with_sms_mock):
    # arrange
    booking_scheduler, sms_sender = booking_scheduler_with_sms_mock
    schedule = Schedule(ON_THE_HOUR, UNDER_CAPACITY, CUSTOMER)
    # act
    booking_scheduler.add_schedule(schedule)
    # assert
    assert sms_sender.send_called

def test_이메일이_없는_경우에는_이메일_미발송(booking_scheduler_with_mail_mock):
    booking_scheduler, mail_sender = booking_scheduler_with_mail_mock
    schedule = Schedule(ON_THE_HOUR, UNDER_CAPACITY, CUSTOMER)
    # act
    booking_scheduler.add_schedule(schedule)
    # assert
...
def test_현재날짜가_일요일인_경우_예약불가_예외처리():
    # arrange
    booking_scheduler = TestableBookingScheduler(CAPACITY_PER_HOUR, SUNDAY_DATETIME)
    schedule = Schedule(ON_THE_HOUR, UNDER_CAPACITY, CUSTOMER)
    # act and assert
    with pytest.raises(ValueError):
        booking_scheduler.add_schedule(schedule)
...
def test_현재날짜가_일요일이_아닌경우_예약가능():
    # arrange
    booking_scheduler = TestableBookingScheduler(CAPACITY_PER_HOUR, MONDAY_DATETIME)
    schedule = Schedule(ON_THE_HOUR, UNDER_CAPACITY, CUSTOMER)
    # act
    booking_scheduler.add_schedule(schedule)
    # assert
